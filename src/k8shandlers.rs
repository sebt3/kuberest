...
use kube::{
    api::{Api, ListParams, ObjectList, PatchParams, PostParams, DeleteParams, Patch},
    Client,
};
use crate::restendpoint::{RESTPATH_FINALIZER, RestEndPoint,Metadata};
...
impl SecretHandler {
    ...
    pub async fn have_uid(&mut self, name: &str, uid: &str) -> bool {
        let list = self.list().await.unwrap();
        for secret in list {
            if secret.metadata.name.clone().unwrap_or_default() == name && secret.metadata.uid.clone().unwrap_or_default() == uid {
                return true;
            }
        }
        false
    ...
    pub async fn create(&mut self, owner:Option<&RestEndPoint>, meta: &Metadata, strings: &HashMap<String,String>) -> Result<Secret, kube::Error> {
...
impl ConfigMapHandler {
    #[must_use] pub fn new(cl: &Client, ns: &str) -> ConfigMapHandler {
        ConfigMapHandler {
            api: Api::namespaced(cl.clone(), ns),
            namespace: ns.to_string()
        }
    ...
    pub async fn have_uid(&mut self, name: &str, uid: &str) -> bool {
        let list = self.list().await.unwrap();
        for cm in list {
            if cm.metadata.name.clone().unwrap_or_default() == name && cm.metadata.uid.clone().unwrap_or_default() == uid {
                return true;
            }
        }
        false
    ...
    pub async fn create(&mut self, owner:Option<&RestEndPoint>, meta: &Metadata, data: &HashMap<String,String>) -> Result<ConfigMap, kube::Error> {
        let mut metadata = serde_json::json!({
            "name": meta.name
        });
        if let Some(own) = owner {

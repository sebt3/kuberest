...
impl RestClient {
    ...
    pub fn body_post(&mut self, path:&str, body: &str) -> Result<String, Error> {
        let response = self.http_post(path,body).or_else(|e| return Err(Error::ReqwestError(e)))?;
        if ! response.status().is_success() {
            let status = response.status();
            let text = tokio::task::block_in_place(|| {Handle::current().block_on(async move {response.text().await})}).or_else(|e| return Err(Error::ReqwestError(e)))?;
    ...
    pub fn obj_update(&mut self, method:UpdateMethod, path: &str, key:&str, input:&Value) -> Result<Value,Error> {
        let full_path = if key=="" {path.to_string()} else {format!("{path}/{key}")};

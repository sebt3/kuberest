use rhai::{Engine, Scope, Module, ImmutableString, Dynamic};
use crate::{Error::*,Error,httphandler::RestClient, passwordhandler::Passwords, handlebarshandler::HandleBars};
...
impl Script {
    #[must_use] pub fn new() -> Script {
        ...
        script.engine.register_fn("log_debug", |s:ImmutableString| tracing::debug!("{s}"))
            ...
            .register_fn("gen_password", |len:u32| -> String {Passwords::new().generate(len, 6, 2, 2)})
            .register_fn("gen_password_alphanum", |len:u32| -> String {Passwords::new().generate(len, 8, 2, 0)})
            .register_fn("get_env", |var: ImmutableString| -> String { std::env::var(var.to_string()).unwrap_or("".into()) })
            .register_fn("base64_decode", |val: ImmutableString| -> ImmutableString { String::from_utf8(STANDARD.decode(val.to_string()).unwrap()).unwrap().into() })
            .register_fn("base64_encode", |val: ImmutableString| -> ImmutableString { STANDARD.encode(val.to_string()).into() })
            .register_fn("json_encode", |val: Dynamic| -> ImmutableString { serde_json::to_string(&val).unwrap().into() })
            .register_fn("json_decode", |val: ImmutableString| -> Dynamic { serde_json::from_str(&val.to_string()).unwrap() })
            .register_fn("yaml_encode", |val: &Dynamic| -> ImmutableString { serde_yaml::to_string(val).unwrap().into() })
            .register_fn("yaml_decode", |val: ImmutableString| -> Dynamic { serde_yaml::from_str(&val.to_string()).unwrap() });
        script.engine.register_type_with_name::<HandleBars>("HandleBars")
        ...
        script.engine.register_type_with_name::<RestClient>("RestClient")
    ...
    pub fn eval(&mut self, script: &str) -> Result<serde_json::Value, Error> {
        match self.engine.eval_with_scope::<rhai::Map>(&mut self.ctx, script) {
            Ok(v) => {
                let value: serde_json::Value = serde_json::from_str(&serde_json::to_string(&v).unwrap()).unwrap();
                Ok(value)
            },
